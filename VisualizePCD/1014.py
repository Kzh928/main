# ファイル名: step3.7_train_with_balanced_batch.py
# 概要: クラス比率を固定したバッチで3クラスモデルの学習と評価を行う。
...
import numpy as np
import pandas as pd
# ▼▼▼【変更】Sampler, BatchSampler をインポート ▼▼▼
...
LABELS_PATH = 'annotated_labels_3class_final_robust_1010.npy'

NUM_NEIGHBORS = 64
NUM_CLASSES = 3
NUM_EPOCHS = 500
# ▼▼▼【変更】バッチサイズは固定比率の合計値とする ▼▼▼
# BATCH_SIZE = 32 # 使用しない
# バッチ内の各クラスのサンプル数 (合計32)
BATCH_RATIO = {
    0: 4,  # 鉄筋 (ラベル1に対応)
    1: 22, # 非鉄筋 (ラベル2に対応)
    2: 6   # 装置 (ラベル3に対応)
...
LEARNING_RATE = 0.001
OUTPUT_MODEL_PATH = 'pointnet_model_3class_final_robust_1010.pth' # ◀◀◀【変更推奨】モデル名を変更
LOG_FILE_PATH = 'training_log_final_robust_1010.csv' # ◀◀◀【変更推奨】ログファイル名を変更
...
class PointCloudDataset(Dataset):
    def __init__(self, points, labels, kdtree):
        ...
        # DataLoaderで使うために、教師データのラベル（0, 1, 2に変換済み）も保持
    ...
    def __getitem__(self, idx):
        ...
        # 近傍点が足りない場合の処理（データ拡張などで対応可能だが、今回は単純にそのまま使う）
        if k < NUM_NEIGHBORS:
            # 足りない場合はランダムに複製して埋める
...
class BalancedBatchSampler(BatchSampler):
    def __init__(self, dataset_labels, batch_ratio):
        ...
        # バッチの総数を決定（最も少ないクラスのサンプル数に基づいて決める）
...
if __name__ == '__main__':
    ...
    print(f"  - 非鉄筋 (クラス1):    {num_not_rebar} 点 ({num_not_rebar/num_total_labeled:.2%})")
    ...
    # ▼▼▼【変更】DataLoaderでbatch_samplerを指定 (shuffleとbatch_sizeは指定しない) ▼▼▼
    # dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True) # 変更前
    dataloader = DataLoader(dataset, batch_sampler=balanced_sampler)
    
    print(f"バッチ比率 -> 鉄筋:{BATCH_RATIO[0]}, 非鉄筋:{BATCH_RATIO[1]}, 装置:{BATCH_RATIO[2]}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # エポックごとの結果計算
        # バランスドサンプリングでは、1エポックで全てのデータを見るわけではない点に注意
    ...
    # ▼▼▼【変更】評価時はバランスを取らず、全データを順番に見る通常のDataLoaderを使う ▼▼▼
