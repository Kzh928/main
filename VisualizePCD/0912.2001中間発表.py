...
# x軸方向で区切るためのインターバル設定（任意の値）
intervals = [x_min, -0.3, -0.1, 0.0, 0.1, 0.3, x_max]  # x_minとx_maxは自動的に最小・最大値
...
for point in data_rotated:
    x_value = point[0]
    # x軸方向の値がどのインターバルに属するかを判断
    for i in range(len(intervals) - 1):
        if intervals[i] <= x_value < intervals[i + 1]:
            colors.append(color_map[i % len(color_map)])  # 色を適用
            break
