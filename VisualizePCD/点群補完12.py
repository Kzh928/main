# 生成日: 2025-06-24
# 目的: 多様な密度を含むデータセットで、点群補完AIを訓練し、
#       様々な入力に対応できる、より頑健なモデルを作成する。
# ファイル名: C4_train_with_repulsion.py (最終学習版)
...
BATCH_SIZE = 32
LEARNING_RATE = 0.001
INPUT_DATASET_PATH = 'multidensity_additive_dataset.pkl'
MODEL_SAVE_PATH = 'robust_upsampler_model.pth'
...
def repulsion_loss(points, k=5, h=0.03):
    ...
    loss = torch.mean(penalty); return loss
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"エラー: データセットファイル '{INPUT_DATASET_PATH}' が見つかりません。ステップA4を先に実行してください。")
        exit()
    
    model = StrictAdditiveGenerator().to(device)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)

    print("\n最終的な点群補完モデルの学習を開始します...")
