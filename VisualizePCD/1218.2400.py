...
#ゆうだい，せいやにいじってもらったやつ
#最終のsvm関連がうごかない
#フィルタリング，回転，反射強度による色の割り振り，表示まではできてる  
#
##
...
for i in range(0,len(filtered_data),1):
    if(filtered_data[i,0] > 0.3 and filtered_data[i,1] < 1.0 and filtered_data[i,2] > 0.5 and  filtered_data[i,2] < 1.5):
...
# 座標軸の作成（確認用）
cs = o3d.geometry.TriangleMesh.create_coordinate_frame(size=2.0)
...
# x_train = filtered_data[:len(filtered_data*0.7)]
# x_test = filtered_data[len(filtered_data*0.7):]
