# ファイル名: step3.7_train_and_evaluate.py
# 概要: 3クラスモデルの学習を行い、学習が完了した直後に、そのモデルの性能を
#      詳細なレポート（混同行列などを含む）として出力する。
...
BATCH_SIZE = 32
LEARNING_RATE = 0.001
OUTPUT_MODEL_PATH = 'pointnet_model_3class_final.pth'
...
if __name__ == '__main__':
    ...
    print(f"  - 鉄筋 (クラス1):     {num_rebar} 点 ({num_rebar/num_total_labeled:.2%})")
    print(f"  - 非鉄筋 (クラス2):   {num_not_rebar} 点 ({num_not_rebar/num_total_labeled:.2%})")
    print(f"  - 装置 (クラス3):     {num_equipment} 点 ({num_equipment/num_total_labeled:.2%})")
    print("--------------------------\n")
    
    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(points)); kdtree = o3d.geometry.KDTreeFlann(pcd)
    dataset = PointCloudDataset(points, labels, kdtree); dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)
