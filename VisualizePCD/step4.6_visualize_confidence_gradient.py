...
if __name__ == '__main__':
    ...
    # 各点の「鉄筋らしさ」の確率を保存するリスト
    rebar_probabilities = np.full(len(points), -1.0, dtype=float)
    ...
    with torch.no_grad():
        for i in tqdm(range(len(points))):
            ...
            rebar_probabilities[i] = rebar_prob
    ...
    valid_indices = np.where(rebar_probabilities != -1.0)[0]
    # RGBAからRGBだけを取り出す
    colors[valid_indices] = cmap(rebar_probabilities[valid_indices])[:, :3]
