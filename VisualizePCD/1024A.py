# ファイル名: step3.7_train_with_balanced_batch_4class_custom_label.py
# 概要: クラス比率を固定したバッチで4クラスモデルの学習と評価を行う。(カスタムラベル対応)
...
# ▼▼▼【注意】4クラス（1:鉄筋, 2:壁面, 3:装置, 5:床）に対応したラベルファイルが必要です ▼▼▼
LABELS_PATH = 'annotated_labels_4class.npy'

NUM_NEIGHBORS = 64
NUM_CLASSES = 4 # クラス数は4で変更なし
NUM_EPOCHS = 500
...
LEARNING_RATE = 0.001
OUTPUT_MODEL_PATH = 'pointnet_model_4class_1235.pth' # ◀◀◀【変更推奨】モデル名を変更
LOG_FILE_PATH = 'training_log_4class_1235.csv' # ◀◀◀【変更推奨】ログファイル名を変更
...
# ▼▼▼【変更】データセットの定義をカスタムラベル(1,2,3,5)に対応 ▼▼▼
class PointCloudDataset(Dataset):
    def __init__(self, points, labels, kdtree):
        ...
        # ラベル値(キー)とクラスID(値)のマッピングを定義
        # 元のラベル (1, 2, 3, 5) を (0, 1, 2, 3) に変換する
        self.label_map = {1: 0, 2: 1, 3: 2, 5: 3}

        # マッピング対象のラベル(1,2,3,5)を持つインデックスのみを対象とする
        self.labeled_indices = np.where(np.isin(self.labels, list(self.label_map.keys())))[0]

        # self.dataset_labels が空にならないようにチェック
        if len(self.labeled_indices) == 0:
            self.dataset_labels = np.array([])
            return

        # DataLoaderで使うために、教師データのラベルをマッピング後の値(0,1,2,3)で保持する
        original_labels = self.labels[self.labeled_indices]
        self.dataset_labels = np.array([self.label_map[l] for l in original_labels])
    ...
    def __getitem__(self, idx):
        ...
        # マッピング辞書を使ってラベルを (0, 1, 2, 3) に変換
        original_label = self.labels[point_idx]
        mapped_label = self.label_map.get(original_label)
...
if __name__ == '__main__':
    ...
    # ▼▼▼【変更】データセットの構成表示を4クラス(ラベル1,2,3,5)対応に ▼▼▼
    print("\n--- データセットの構成 ---")
    target_labels = [1, 2, 3, 5]
    labeled_indices = np.where(np.isin(labels, target_labels))[0]
    num_total_labeled = len(labeled_indices)
    
    num_rebar = np.sum(labels == 1)
    num_wall = np.sum(labels == 2)
    num_equipment = np.sum(labels == 3)
    num_floor = np.sum(labels == 5) # ◀◀◀ ラベル5をカウント

    print(f"ラベル付き総サンプル数: {num_total_labeled}")
    if num_total_labeled > 0:
        print(f" - 鉄筋 (ラベル1 -> クラス0):         {num_rebar} 点 ({num_rebar/num_total_labeled:.2%})")
        print(f" - コンクリート壁面 (ラベル2 -> クラス1): {num_wall} 点 ({num_wall/num_total_labeled:.2%})")
        print(f" - 装置 (ラベル3 -> クラス2):         {num_equipment} 点 ({num_equipment/num_total_labeled:.2%})")
        print(f" - 床 (ラベル5 -> クラス3):           {num_floor} 点 ({num_floor/num_total_labeled:.2%})")
    print("--------------------------\n")
    ...
    # バッチ比率の表示 (ラベル5に対応するクラス3が対象)
    print(f"バッチ比率 -> 鉄筋(0):{BATCH_RATIO[0]}, 壁面(1):{BATCH_RATIO[1]}, 装置(2):{BATCH_RATIO[2]}, 床(3):{BATCH_RATIO[3]}")
    ...
    # クラス名はクラスID(0,1,2,3)に対応するため変更不要
    ...
    for i, row in enumerate(cm):
        if i < len(target_names):
            ...
            for j in range(len(target_names)):
                val = cm[i, j] if j < cm.shape[1] else 0
                row_str += f" {val:11d}"
            print(row_str)
    print(" " + "-" * (len(header)-1) + "\n")
    print("===== 全ての処理が完了しました =====")