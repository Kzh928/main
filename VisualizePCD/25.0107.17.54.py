...
#特徴量の部分で反射強度の比重を大きくして見る
#特徴点の比重を変更できる
#片方を0にすることでもう片方の特徴点のみを扱うこともできる
#
#
#
#
...
#  coordinate_weight = 1.0，intensity_weight = 2.0 に設定すると，
#  反射強度が座標の2倍重視される．
...
intensity = filtered_data[:,3].reshape(-1,1)

# 重みを適用した特徴量の統合
weighted_coordinates = coordinates * coordinate_weight
weighted_intensity = intensity * intensity_weight

# 統合された特徴量
features = np.hstack((weighted_coordinates, weighted_intensity))
...
# --- サンプリング処理（追加部分） ---
max_samples = 20000  # SVM に使用する最大サンプル数
...
clf = OneClassSVM(nu=0.2, kernel='rbf', gamma='scale')  # gamma='scale'は推奨
...
print
Display = float(input("Output confirmation: "))
...
num_anomalous = len(anomalous_data)
total_points = num_normal + num_anomalous

print(f"Number of points displayed:")
print(f"  Normal points: {num_normal}")
print(f"  Anomalous points: {num_anomalous}")
...
ax.scatter(normal_data[:, 0], normal_data[:, 1], normal_data[:, 2], label="Normal", color="blue",s=point_size)
ax.scatter(anomalous_data[:, 0], anomalous_data[:, 1], anomalous_data[:, 2], label="Anomalous", color="red",s=point_size
           )
...
ax.set_zlabel("z")
plt.legend()
plt.title("3D Detection of Rebar vs Non-Rebar(with Intensity)")
plt.show()
