# ファイル名: step9.0_chipping_area_mapping.py
# 概要: 壁面検出、3クラス分類、はつり範囲特定までを全自動で行う統合パイプライン。
#       最後に4色表示と、はつり範囲のみをハイライトした2色表示を行う。
...
if __name__ == '__main__':
    ...
    # フェーズ1: 領域成長による壁面検出
    # =================================================================
    pcd_world = load_and_prepare_pcd(FULL_PCD_PATH)
    ...
    if len(rebar_indices) > 0 and len(non_rebar_indices) > 0:
        rebar_points_yz = points_wall[rebar_indices][:, 1:]; rebar_kdtree_2d = cKDTree(rebar_points_yz)
        non_rebar_points_yz = points_wall[non_rebar_indices][:, 1:]
        distances, nearest_rebar_indices_in_list = rebar_kdtree_2d.query(non_rebar_points_yz, k=1)
        non_rebar_x = points_wall[non_rebar_indices][:, 0]
        nearest_rebar_x = points_wall[rebar_indices[nearest_rebar_indices_in_list]][:, 0]
        chipping_mask = non_rebar_x < nearest_rebar_x
    ...
    o3d.visualization.draw_geometries([pcd_wall_only], window_name="最終結果(1/2): 詳細分類（4色）")
    ...
    # 「削らない」点を灰色、「削る」点を黄色に塗り分ける
    colors_binary = np.full((len(points_wall), 3), [0.5, 0.5, 0.5]) # デフォルトは灰色
    colors_binary[final_wall_labels == LABEL_CHIPPING] = [1.0, 1.0, 0.0] # 削るべき点(4)を黄色に
    
    pcd_wall_only.colors = o3d.utility.Vector3dVector(colors_binary)
    o3d.visualization.draw_geometries([pcd_wall_only], window_name="最終結果(2/2): はつり範囲（2色）")
    # === ▲▲▲【ここまで追記】▲▲▲ ===
