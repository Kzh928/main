...
#
#クラスタリング前に扱うデータをyz平面で微小区間に区切るやつに
#してみた，扱うデータ数少なくしたら行けるかなみたいな
#
#
#結果だけいくと無理
#一番手前を選択する段階で一定はんいごとに一点を選ぶようにしてたから
#閾値に何も引っかからなくなった，全部が別のクラスタ所属
#
#区切る範囲そんなに大きくせずにしてみたら鉄筋に何点か当たるのでは
#
...
for y_start in np.arange(y_min, y_max, interval):
    for z_start in np.arange(z_min, z_max, interval):
        # 現在の区間に属する点を抽出
        mask = (data_rotated[:, 1] >= y_start) & (data07[:, 1] < y_start + interval) & \
               (data_rotated[:, 2] >= z_start) & (data07[:, 2] < z_start + interval)
        section_points = data_rotated[mask]
        
        if len(section_points) > 0:
            # x軸方向で最も手前の点を選択
            nearest_point = section_points[np.argmin(section_points[:, 0])]
            selected2_points.append(nearest_point)
            
            # 色を設定
            color = color_map[(int(y_start / interval) + int(z_start / interval)) % len(color_map)]
            selected2_colors.append(color)

pcd_selected2 = o3d.geometry.PointCloud()
pcd_selected2.points = o3d.utility.Vector3dVector(selected2_points)

# 選択したデータの数の表示
print(len(selected2_points))
...
data_for_clustering = np.array(selected2_points)
...
db = DBSCAN(eps=10, min_samples=5).fit(data_for_clustering)
