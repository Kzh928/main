...
# 概要: RANSACベースの頑健な法線推定を導入し、より正確な領域成長を目指す。
#       法線計算に非常に時間がかかるため注意。
...
def estimate_normals_robust(pcd, radius, max_nn, num_iterations, angle_thresh_deg=15.0):
    ...
    for i in tqdm(range(len(points)), desc="ロバスト法線計算中"):
        ...
        if k < 3: continue # 3点未満では平面を定義できない

        neighbor_points = points[neighbor_indices]
        candidate_normals = []

        # 指定された回数だけランダムサンプリングを実行
...
# (get_initial_seeds, calculate_growth_history, AnimationState, etc. の部分は変更なし)
...
def calculate_growth_history(pcd, seed_indices, normal_angle_thresh_deg, search_radius, save_interval):
    ...
    history.append(list(wall_indices)); print("計算が完了しました
    。"); return history
class AnimationState:
    def __init__(self, full_pcd, history_world):
        self.full_pcd_points = np.asarray(full_pcd.points)
        self.history = history_world; self.current_frame = 0; self.max_frame = len(history_world) - 1
        self.pcd_to_display = o3d.geometry.PointCloud(); self.update_geometry()
    def update_geometry(self):
        ...
        points = self.full_pcd_points[current_indices]; colors = np.full_like(points, [0.8, 0.4, 0.4])
        new_point_mask = np.isin(current_indices, list(previous_indices), invert=True)
        colors[new_point_mask] = [1.0, 0.0, 0.0]
        self.pcd_to_display.points = o3d.utility.Vector3dVector(points); self.pcd_to_display.colors = o3d.utility.Vector3dVector(colors)
        print(f"フレーム {self.current_frame}/{self.max_frame} を表示中 (点の数: {len(points)})")
def go_to_next_frame(vis, state):
    if state.current_frame < state.max_frame: state.current_frame += 1; state.update_geometry(); vis.update_geometry(state.pcd_to_display)
def go_to_prev_frame(vis, state):
    if state.current_frame > 0: state.current_frame -= 1; state.update_geometry(); vis.update_geometry(state.pcd_to_display)
...
if __name__ == '__main__':
    ...
    # ▼▼▼【追加】頑健な法線推定用のパラメータ ▼▼▼
    ROBUST_NORMAL_ITERATIONS = 50 # 1点あたりのサンプリング回数。10~50程度が目安。
