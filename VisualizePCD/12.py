...
from scipy.spatial import cKDTree # ◀◀◀【追加】高速な近傍点探索ライブラリ
...
if __name__ == '__main__':
    ...
    points_wall = np.asarray(pcd_wall_only.points)
    kdtree_wall = o3d.geometry.KDTreeFlann(pcd_wall_only)
    ...
    # 鉄筋(0), 非鉄筋(1), 装置(2) のインデックスを取得
    rebar_indices = np.where(wall_labels == 0)[0]
    non_rebar_indices = np.where(wall_labels == 1)[0]
    
    final_wall_labels = np.copy(wall_labels) # 最終的なラベル配列
    
    if len(rebar_indices) > 0 and len(non_rebar_indices) > 0:
        # 鉄筋の YZ 座標だけで2DのKDTreeを作成
        rebar_points_yz = points_wall[rebar_indices][:, 1:] # YとZだけを抽出
    ...
    o3d.visualization.draw_geometries([pcd_wall_only], window_name="最終結果: はつり範囲のマッピング")
