# ファイル名: step4.6_visualize_confidence_gradient.py
...
if __name__ == '__main__':
    ...
    # 各点の「非鉄筋らしさ」の確率を保存するリスト
    non_rebar_probabilities = np.full(len(points), -1.0, dtype=float)

    print("推論を実行し、各点の「鉄筋らしさ」の確率を取得中...")
    with torch.no_grad():
        for i in tqdm(range(len(points))):
            ...
            # log_softmaxの出力を確率に変換 (torch.exp)
            probabilities = torch.exp(outputs)
            # --- 変更後のコード ---
            # 「非鉄筋」クラス(インデックス1)の確率を取得
            non_rebar_prob = probabilities[0, 1].cpu().numpy()
            # 変数名も分かりやすく変更すると良いでしょう
            non_rebar_probabilities[i] = non_rebar_prob
           

    print("推論完了。確率に応じて色を付け、可視化します。")
    ...
    valid_indices = np.where(non_rebar_probabilities != -1.0)[0]
    # RGBAからRGBだけを取り出す
    colors[valid_indices] = cmap(non_rebar_probabilities[valid_indices])[:, :3]
    ...
    o3d.visualization.draw_geometries(
        [result_pcd, axis],
        window_name="Prediction Confidence (Gradient Color)",
