# ファイル名: D2_test_additive_model.py
...
# --- ★★★ ユーザー設定項目 ★★★ ---
MODEL_PATH = 'additive_model.pth'
DATASET_PATH = 'additive_dataset.pkl'
NUM_SAMPLES_TO_SHOW = 100
X_SPACING = 200.0
Y_SPACING = 200.0

# --- モデル定義 (C2の学習スクリプトと全く同じ) ---
...
if __name__ == '__main__':
    ...
    # 2. テスト用データセットの準備
    try:
        dataset = AdditiveDataset(DATASET_PATH)
        if len(dataset) < NUM_SAMPLES_TO_SHOW:
            NUM_SAMPLES_TO_SHOW = len(dataset)
        test_samples = random.sample(dataset.data, NUM_SAMPLES_TO_SHOW)
    ...
    geometries_to_draw = []
    print("AIに32点から64点の点群を生成させています...")
    with torch.no_grad():
        for i, (sparse_points_np, dense_points_truth_np) in enumerate(test_samples):
            input_tensor = torch.from_numpy(sparse_points_np).unsqueeze(0).to(device)
            # AIによる点群生成（推論）
            generated_combined_tensor = model(input_tensor)
            generated_combined_points = generated_combined_tensor.squeeze(0).cpu().numpy()

            y_offset = -i * Y_SPACING

            # 入力 (青)
            pcd_input = o3d.geometry.PointCloud()
            pcd_input.points = o3d.utility.Vector3dVector(sparse_points_np)
    ...
    o3d.visualization.draw_geometries(
        geometries_to_draw,
        window_name=f"{NUM_SAMPLES_TO_SHOW} Additive Model Test Results",
