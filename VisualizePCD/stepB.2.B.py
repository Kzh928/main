# ファイル名: wall_detection_final_with_legend.py
# 概要: 最終結果を表示する前に、各色が何を表すかの
#       凡例をコンソールに表示する機能を追加。
...
if __name__ == '__main__':
    ...
    wall_segments_local, final_wall_local = expand_by_iterative_ransac(
        ...
        coloring_ranges=COLORING_RANGES
    )
    ...
    # ▼▼▼【ここから変更】凡例を表示してから、ステップ3の表示を開始 ▼▼▼
    print("\n【ステップ3】指定された範囲で、最終的な壁面を連続表示します。")
    
    # カラー凡例の表示
    print("\n" + "="*22 + " カラー凡例 " + "="*22)
    print(f"| {'色'.ljust(12)} | {'内容'.ljust(40)} |")
    print(f"|{'-'*14}|{'-'*42}|")
    print(f"| {'赤 (Red)'.ljust(15)} | {'核 (アルゴリズムの起点となる最初の平面)'.ljust(40)} |")
    if COLORING_RANGES:
        for start, end, color_val in COLORING_RANGES:
            # 簡単な色名判定
            color_name = f"カスタム色 {color_val}"
            if color_val == [0.0, 1.0, 0.0]: color_name = "緑 (Green)"
            elif color_val == [0.0, 0.0, 1.0]: color_name = "青 (Blue)"
            elif color_val == [1.0, 1.0, 0.0]: color_name = "黄 (Yellow)"
            
            description = f"ループ {start}～{end} 回目の拡張部分"
            print(f"| {color_name.ljust(15)} | {description.ljust(40)} |")
    print(f"| {'紫 (Purple)'.ljust(15)} | {'上記以外のループでの拡張部分 (デフォルト色)'.ljust(40)} |")
    print("="*58)
