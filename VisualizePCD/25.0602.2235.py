...
def apply_clahe_to_reflectance(reflectance_values, clip_limit=2.0, tile_grid_size=(8, 8)):
    # 反射強度を 0-255 に正規化して8bit整数に変換
    reflectance_scaled = ((reflectance_values - reflectance_values.min()) / 
                          (reflectance_values.max() - reflectance_values.min() + 1e-6) * 255).astype(np.uint8)
    
    # CLAHEオブジェクトの生成と適用（擬似1D画像として reshape）
    clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=tile_grid_size)
    reflectance_eq = clahe.apply(reflectance_scaled.reshape(-1, 1)).flatten()
    
    # 元のスケール（0-1）に戻す
    reflectance_eq_norm = reflectance_eq.astype(np.float32) / 255.0
    return reflectance_eq_norm
...
if filtered_pca_points.shape[0] > 0:
    # CLAHE的強調を適用
    reflect_norm_clahe = apply_clahe_to_reflectance(filtered_pca_reflect)
    
    # カラーマップで色変換
    colormap = cm.get_cmap("jet")
    colors_mapped = colormap(reflect_norm_clahe)[:, :3]
