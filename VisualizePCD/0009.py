# ファイル名: step3.3_train_with_weighted_loss.py
# 概要: データの不均衡を解消するため、サンプル数が少ないクラスほどペナルティが
#      大きくなる「重み付き損失関数」を導入して3クラスモデルを学習する。
...
BATCH_SIZE = 32
LEARNING_RATE = 0.001
OUTPUT_MODEL_PATH = 'pointnet_model_3class_weighted.pth' # ★変更: 新しいモデル名
LOG_FILE_PATH = 'training_log_weighted.csv'           # ★変更: 新しいログ名
...
if __name__ == '__main__':
    ...
    # ▼▼▼【新機能】クラスの重みを計算 ▼▼▼
    print("\nクラスの重みを計算しています...")
    # 学習対象となるラベルだけを抽出 (0-indexedに変換)
    train_labels = labels[np.where(labels > 0)[0]] - 1
    # scikit-learnのユーティリティを使って、クラスの重みを計算
    class_weights = compute_class_weight('balanced', classes=np.unique(train_labels), y=train_labels)
    class_weights = torch.tensor(class_weights, dtype=torch.float)
    
    print("計算されたクラスの重み:")
    print(f"  - 鉄筋 (クラス0): {class_weights[0]:.4f}")
    print(f"  - 非鉄筋 (クラス1): {class_weights[1]:.4f}")
    print(f"  - 装置 (クラス2): {class_weights[2]:.4f}")
    ...
    print("重み付き損失関数 (Weighted NLLLoss) を使用します。")
