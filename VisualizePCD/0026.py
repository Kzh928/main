# ファイル名: step3.5_train_with_balanced_sampling.py
# 概要: クラス間のデータ量の不均衡を解消するため、WeightedRandomSamplerを用いた
#      バランスサンプリングを導入し、3クラスモデルを学習する。
...
NUM_NEIGHBORS = 64
NUM_CLASSES = 3
NUM_EPOCHS = 5 # エポック数は少し多めに設定しても良い
BATCH_SIZE = 32
LEARNING_RATE = 0.001
OUTPUT_MODEL_PATH = 'pointnet_model_3class_balanced.pth'
LOG_FILE_PATH = 'training_log_balanced.csv'
...
if __name__ == '__main__':
    ...
    # 各サンプルに対して、そのクラスの重みを割り当てる
    sample_weights = class_weights[train_labels]
    
    # WeightedRandomSamplerを定義
    sampler = WeightedRandomSampler(weights=sample_weights, num_samples=len(sample_weights), replacement=True)
    
    # DataLoaderにsamplerを設定 (shuffle=Trueは使わない)
    ...
    # 損失関数は通常のNLLLossを使う (重みはサンプラーで解決するため)
