# ファイル名: step10.4_visualize_rebar_plane.py
# 概要: はつり範囲の基準となった「仮想の鉄筋平面」を可視化する。(可視化エンジン修正版)
...
# (PointNetモデル定義、壁面検出関数群は変更ないため省略します)
# ... (step10.4と同じPointNetモデル定義と壁面検出関数をここにコピーしてください) ...
...
def create_plane_mesh(plane_model, center, size=2000):
    [a, b, c, d] = plane_model; normal = np.array([a, b, c])
    u = np.cross(normal, [0, 0, 1]);
    if np.linalg.norm(u) < 1e-6: u = np.cross(normal, [0, 1, 0])
    u /= np.linalg.norm(u); v = np.cross(normal, u)
    half_size = size / 2
    p0 = center - half_size * u - half_size * v; p1 = center + half_size * u - half_size * v
    p2 = center + half_size * u + half_size * v; p3 = center - half_size * u + half_size * v
    mesh = o3d.geometry.TriangleMesh()
    mesh.vertices = o3d.utility.Vector3dVector([p0, p1, p2, p3])
    mesh.triangles = o3d.utility.Vector3iVector([[0, 1, 2], [0, 2, 3]])
    mesh.paint_uniform_color([0, 1, 0]); return mesh

# --- メイン処理 ---
if __name__ == '__main__':
    # ... (パラメータ設定、フェーズ1, 2は変更なし) ...
    ...
    # フェーズ3: はつり範囲のマッピング と 平面の可視化準備
    print("\n--- フェーズ3: はつり範囲のマッピング (最も奥の平面ロジック) ---")
    final_wall_labels = np.copy(wall_labels); LABEL_CHIPPING = 4
    plane_mesh = None
    rebar_indices_in_wall = np.where(wall_labels == 0)[0]
    non_rebar_indices_in_wall = np.where(wall_labels == 1)[0]
    if len(rebar_indices_in_wall) > 3 and len(non_rebar_indices_in_wall) > 0:
        rebar_points = points_wall[rebar_indices_in_wall]
        max_x = np.max(rebar_points[:, 0])
        rearmost_rebar_indices = rebar_indices_in_wall[rebar_points[:, 0] >= max_x - REARMOST_PLANE_THRESHOLD]
        if len(rearmost_rebar_indices) < 3:
            print("警告: 最も奥の平面を計算するための鉄筋が少なすぎます。")
        else:
            print(f"{len(rearmost_rebar_indices)}点の最も奥にある鉄筋から仮想平面を計算中...")
            pcd_rearmost_rebar = pcd_wall_only.select_by_index(rearmost_rebar_indices)
            plane_model, _ = pcd_rearmost_rebar.segment_plane(distance_threshold=20.0, ransac_n=3, num_iterations=1000)
            [a, b, c, d] = plane_model
            print(f"  -> 最も奥の平面の方程式: {a:.3f}x + {b:.3f}y + {c:.3f}z + {d:.3f} = 0")
            plane_mesh = create_plane_mesh(plane_model, pcd_rearmost_rebar.get_center())
            points_non_rebar = points_wall[non_rebar_indices_in_wall]
            x_rebar_plane = (-b * points_non_rebar[:, 1] - c * points_non_rebar[:, 2] - d) / a
            x_non_rebar = points_non_rebar[:, 0]
            chipping_mask = x_non_rebar < x_rebar_plane
            chipping_indices_in_wall = non_rebar_indices_in_wall[chipping_mask]
    ...
    geometries_to_draw = [pcd_wall_only]
    if plane_mesh is not None:
        plane_mesh.compute_vertex_normals()
        material = o3d.visualization.rendering.MaterialRecord()
        material.shader = "defaultLitTransparency"
        material.base_color = [0.0, 1.0, 0.0, 0.5]
        material.base_roughness = 0.5
        geometries_to_draw.append({"name": "plane", "geometry": plane_mesh, "material": material})
    
    print("\n詳細な分類結果と、基準となった仮想平面（緑の半透明な板）を表示します。")
    o3d.visualization.draw(geometries_to_draw, title="最終結果: 詳細分類 + 仮想平面")
    
    print("\n次に「削る/削らない」のシンプルな判断結果を表示します。")
    ...
    # ▼▼▼【変更】古い関数から新しい関数に統一 ▼▼▼
    o3d.visualization.draw([pcd_wall_only], title="最終結果: はつり範囲 (最奥平面版)")
