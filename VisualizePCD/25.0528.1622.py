...
# --- CSVデータ読み込み（BOM対策） ---
filename = '2025.03.12.00.53.53.csv'
...
center_point = filtered_points[center_idx]

# --- KDTreeを使って近傍4点を取得 ---
tree = KDTree(filtered_points)
_, neighbor_indices = tree.query(center_point, k=5)  # k=5（自身＋近傍4点）

# --- 近傍点の抽出 ---
selected_points = filtered_points[neighbor_indices]

# --- 近傍点との距離を計算 ---
neighbor_distances = np.linalg.norm(selected_points - center_point, axis=1)

# --- 距離を表示 ---
print(f"centor: {center_point}")
