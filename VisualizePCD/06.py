# ファイル名: step8.1_detect_rebar_in_wall.py
# 概要: 最初に領域成長で壁面を特定し、抽出された壁面点群に対してのみ
#       PointNetによる3クラス分類（鉄筋/非鉄筋/装置）を実行する統合パイプライン。
...
if __name__ == '__main__':
    ...
    if pcd_wall_only.is_empty():
        exit("エラー: 抽出された壁の点群が空です。")

    # =================================================================
    # フェーズ2: 抽出された壁面内での鉄筋検出
    # =================================================================
    print("\n--- フェーズ2: 壁面内の鉄筋検出 ---")
    print("PointNetモデルを読み込んでいます...")
    ...
    wall_colors = np.full((len(points_wall), 3), [0.5, 0.5, 0.5]) # デフォルトは灰色
    color_map = {
        0: [1.0, 0.0, 0.0], # 鉄筋 -> 赤
        1: [0.0, 0.0, 1.0], # 非鉄筋 -> 青
        2: [0.0, 1.0, 0.0]  # 装置 -> 緑
    ...
    with torch.no_grad():
        # 壁と特定された点に対してのみループ
        for i in tqdm(range(len(points_wall)), desc="鉄筋検出中"):
            # KDTreeも壁の点群から作成したものを使用
            [k, idx, _] = kdtree_wall.search_knn_vector_3d(points_wall[i], NUM_NEIGHBORS)
            if k < NUM_NEIGHBORS: continue
            
            neighbors = points_wall[idx]
            normalized_neighbors = neighbors - points_wall[i]
            input_tensor = torch.from_numpy(normalized_neighbors.T).float().unsqueeze(0).to(device)
            
            outputs = model(input_tensor)
            _, predicted_label = torch.max(outputs.data, 1)
            
            wall_colors[i] = color_map[predicted_label.item()]

    # =================================================================
    # フェーズ3: 最終結果の可視化
    # =================================================================
    print("\n処理完了。最終結果を表示します。")
    pcd_wall_only.colors = o3d.utility.Vector3dVector(wall_colors)
    o3d.visualization.draw_geometries([pcd_wall_only], window_name="統合結果: 壁面内の鉄筋検出")
